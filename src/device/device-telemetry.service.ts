...
export class DeviceTelemetryService {
  ...
  async handleTelemetry(telemetry: IncomingTelemetry) {
    ...
    this.logger.error(`[HISTORY] ${JSON.stringify(telemetry.data.historicalTelemetry, null, 2)}`);
    ...
    Object.entries(telemetry.data).forEach(([field, value]) => {
      ...
      mergedData[field].push([value, telemetry.timestamp]);
    });
    ...
    await this.deviceRepository.deleteOldTelemetryForDevice(telemetry.deviceId, 5);
