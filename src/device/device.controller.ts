...
async sendDeviceCommand( @Param('id') id: string,
  ...
  const rawClientStartedAt = req.headers?.['x-ui-command-started-at'];
  const clientStartedAt = Number(
    Array.isArray(rawClientStartedAt)
      ? rawClientStartedAt[0]
      : rawClientStartedAt,
  );
  const commandPerformance =
    Number.isFinite(clientStartedAt) && clientStartedAt > 0
      ? {
          clientStartedAt,
          serverReceivedAt,
          uiToServerMs: Math.max(
            0,
            serverReceivedAt - clientStartedAt,
          ),
        }
      : undefined;
