...
export class DeviceService {
  ...
  async applyModelVersion(
    ...
    const targetVersion = await this.repository.findModelVersionById(
    ...
    try {
      ...
      const stageResponse = await this.commandService.sendCommandAndWaitForResponse(
        device.serialNumber,
        'STAGE_MODEL_VERSION',
        {
      ...
      if (!stageResponse.success) {
        ...
        throw new ConflictException(
          stageResponse.error ?? 'DEVICE_REJECTED_MODEL_VERSION',
        );
      }
      ...
      await this.dashboardPlugin.invalidateDeviceCache(
        device.serialNumber,
      );
      ...
      const restartResponse = await this.commandService.sendCommandAndWaitForResponse(
        device.serialNumber,
      ...
      if (!restartResponse.success) {
        throw new ConflictException(
          restartResponse.error ?? 'DEVICE_RESTART_REJECTED',
        );
      }
