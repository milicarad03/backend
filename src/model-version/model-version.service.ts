...
export class ModelVersionService {
  ...
  async uploadVersion(
    ...
    const schema = this.parseJsonFile(schemaFile.buffer, 'SCHEMA');
    const mapping = this.parseJsonFile(mappingFile.buffer, 'MAPPING');
    ...
    if (!validation.valid) {
      ...
      throw new BadRequestException({
        message: 'SCHEMA_MAPPING_COMPATIBILITY_FAILED',
        errors: validation.errors,
      });
    }

    const existingVersion = await this.repository.findOne({
      modelId_version: {
    ...
    if (existingVersion) {
      throw new ConflictException('MODEL_VERSION_ALREADY_EXISTS');
    }
